...
```{r}
not_filtered <- open_dataset("labevents.csv", format = "csv")
system.time(
  labevents_pq <- write_dataset(not_filtered, path = "pq_files", format = "parquet") |>
    select(subject_id, itemid, charttime, valuenum) |>
    filter(itemid %in% c(50862, 50912, 50971, 50983, 50902, 50882, 51221, 51301, 50931)) |>
    collect()
)
...
tibble(
  files = list.files("pq_files", recursive = TRUE),
  size_MB = file.size(file.path("pq_files", files)) / 1024^2
)
